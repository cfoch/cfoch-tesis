...
\newglossaryentry{GObject}
{
    name=GObject,
    description={Una librería de GNOME escrita en C que implementa un sistema
                 orientado a objetos. Una amplia variedad de clases heredan de
                 GObject.}
}
\newglossaryentry{gstopencvvideofilter}
{
    name=gstopencvvideofilter,
    description={Un filtro de GStreamer escrito con el propósito de ser heredado
                 por otros filtros como por ejemplo gstfacedetect. Este filtro
                 recibe una imagen en un buffer y lo encapsula en un tipo de
                 dato reconocido por OpenCV.}
}
\newglossaryentry{gstfacedetect}
{
    name=gstfacedetect,
    description={Un filtro de GStreamer para la detección de rostros usando
                 internamente el detector de objetos de Viola Jones de OpenCV.}
}
\newglossaryentry{gstvideotestsrc}
{
    name=gstvideotestsrc,
    description={Un elemento de GStreamer tipo \textit{source} que produce
                 datos de prueba. Las imágenes producidas corresponden a un
                 patrón de señal de ajuste ampliamente usado para pruebas de
                 video en televisión.}
}
\newglossaryentry{gstcairooverlay
{
    name=\gstcairooverlay,
    description={Un filtro de GStreamer que permite dibujar sobre cada cuadro
                 del stream usando la librería cairo.}
}
\newglossaryentry{GtkToggleButton}
{
    name=GtkToggleButton,
    description={Un widget de Gtk+ que dibuja un botón de dos estados
                 que se comporta como un interruptor.}
}
\newglossaryentry{signals}
{
    name=signals,
    description={GObject tiene un mecanismo basado en señales. Cada vez que una
                 señal es emitida una función de retrollamada se activa. Por
                 ejemplo, GtkButton emite una señal ``clicked'' cada vez que el
                 usuario hace clic sobre el botón. Las señales de GObject no
                 tienen relación alguna con las señales de UNIX.}
}
\newglossaryentry{landmark}
{
    name=landmark,
    description={Es un punto significativo de un objeto u organismo que es comun
                 a su clase o especie.},
    plural=landmarks
}
...
\subsubsection{Gráficas de precisión, exhaustividad y valor-F por cuadro y por
    ...
    \begin{figure}[h]
      \centering
        \includegraphics{../images/multiple-camera1-precision-1.png}\par
    ...
    \begin{figure}[h]
      \centering
        \includegraphics{../images/multiple-camera1-recall-1.png}\par
    ...
    \begin{figure}[h]
      \centering
        \includegraphics{../images/multiple-camera1-f-1.png}\par
    ...
    \begin{figure}[h]
      \centering
        \includegraphics{../images/multiple-camera1-precision-2.png}\par
    ...
    \begin{figure}[h]
      \centering
        \includegraphics{../images/multiple-camera1-recall-2.png}\par
    ...
    \begin{figure}[h]
      \centering
        \includegraphics{../images/multiple-camera1-f-2.png}\par
